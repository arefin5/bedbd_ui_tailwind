...
//         <div
//           className="bg-blue-500"
//           style={{ height: `${(booking.price / 3000) * 100}%`, width: '20px' }}
//         ></div>
...
//     <div className='pt-4 pb-6 pl-4 w-full'>
//       <div>
//         <h3 className='text-neutral-500 text-lg font-semibold'>Revenue by Check-in Date</h3>
//       </div>
//       <div className='relative w-full'>
//         <div className='relative h-48 flex flex-col-reverse justify-between '>
//           <span className='text-xs absolute text-neutral-400' style={{ bottom: `-8px`, right: 'calc(100% - 36px)' }}>0</span>
...
//       <div className='mt-8 flex items-center justify-center gap-4'>
//         <div className='min-h-2 min-w-4 h-2 w-4 bg-secondary-400 '/>
//         <span className='text-neutral-500 text-xs font-medium'>Revenue</span>
//       </div>
//     </div>
//   );
// }
...
export default function RevenueChart({ bookings }) {
  ...
  // Calculate the maximum price from the bookings
  const maxPrice = Math.max(...bookings.map(booking => booking.price), 1); // Fallback to 1 to avoid division by zero
  ...
  const renderBars = () => {
    return bookings.map((booking, index) => (
      <div key={index} className="flex flex-col items-center">
        <div
          className="bg-blue-500"
          style={{ height: `${(booking.price / maxPrice) * 100}%`, width: '20px' }}
        ></div>
        <span className="text-xs mt-2">
          {new Date(booking.checkinDate).toLocaleDateString('en-US')}
        </span>
